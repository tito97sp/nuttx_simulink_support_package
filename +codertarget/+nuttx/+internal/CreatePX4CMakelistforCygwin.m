function CreatePX4CMakelistforCygwin(hCS,buildInfo)
...
for i = 1:numel(FullfilePaths_List)
    ...
    if strcmp(ext,'.c') || strcmp(ext,'.cpp') || strcmp(ext,'.s') || strcmp(ext,'.lib') || strcmp(ext,'.a') || strcmp(ext,'.h')
...
for FileIdx = 1:length(FullfilePathsFiltered_List)
    [filePath, ~, ~] = fileparts(FullfilePathsFiltered_List{FileIdx});
    if ~isempty(filePath)
...
hs.addcr('## the UAVToolbox Support Package for PX4 Autopilots');
hs.addcr();
...
hs.addcr('    INCLUDES');
hs.addcr(sprintf('       %s', sprintf("${PX4_SOURCE_DIR}/src/modules/px4_simulink_app")));
