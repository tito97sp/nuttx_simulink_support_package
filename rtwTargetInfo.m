...
function config = loc_createToolchain
    ...
    arch = computer('arch') ;
    config(end).Name           = 'GNU Tools for PX4 Autopilots';
    config(end).Alias          = ['GNU_PX4_NUTTX_' upper(arch)]; % internal use only
...
function isConfigSetCompatible = i_isConfigSetCompatible(configSet)
    ...
    if configSet.isValidParam('CoderTargetData')
        ...
        isConfigSetCompatible = (isequal(coderTargetData.TargetHardware , message('px4:hwinfo:Pixhawk1').getString))|| ...
            (isequal(coderTargetData.TargetHardware , message('px4:hwinfo:Pixhawk2').getString)) || ...
            (isequal(coderTargetData.TargetHardware , message('px4:hwinfo:Pixhawk4').getString)) || ...
            (isequal(coderTargetData.TargetHardware , message('px4:hwinfo:PixRacer').getString)) || ...
            (isequal(coderTargetData.TargetHardware , message('px4:hwinfo:PixhawkSeries').getString)) || ...
            (isequal(coderTargetData.TargetHardware , message('px4:hwinfo:Crazyflie2_0').getString));
    end
end
